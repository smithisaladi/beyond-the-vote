...
import { useFetchRepresentatives, type Representative } from '@/hooks/useFetchRepresentatives'
...
function RepresentativesContent() {
  ...
  const [nameResults, setNameResults] = useState<Representative[]>([])
  ...
  useEffect(() => {
    ...
    if (nameQuery.length < 3) { setNameResults([]); return }
    nameTimerRef.current = setTimeout(async () => {
      setNameLoading(true)
      try {
        const res = await fetch(`/api/politicians/search?q=${encodeURIComponent(nameQuery)}`)
        const data = await res.json()
        setNameResults(data.politicians ?? [])
