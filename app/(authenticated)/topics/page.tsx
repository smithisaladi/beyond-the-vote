    ...
    >
      <div className="bg-white rounded-xl border border-[#D6CFC4] px-6 py-4 flex items-start gap-4 hover:shadow-sm transition-shadow">
        <div className="flex-1 min-w-0">
          ...
          <Link
            href={`/topics/${topicToSlug(bill.topic)}`}
...
export default function TopicsPage() {
  ...
  useEffect(() => {
    ...
    const fetches = topics.map(async (topic) => {
      const url = `/api/bills/by-topic?slug=${topicToSlug(topic)}&limit=4`
      try {
        const res = await fetch(url)
        if (!res.ok) return []
        const data = await res.json()
        return (data.bills ?? []).map((b: ApiBill) => ({ ...b, topic }))
      } catch {
        return []
      }
    })
    ...
    Promise.all(fetches).then(results => {
      ...
      const merged = results.flat().filter((b: ApiBill) => {
