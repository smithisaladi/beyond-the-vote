 ...
 * Source: NIST SP 800-87 + Federal Register agency list.
 *
 * Design: map all known variant forms to one canonical string so that
 * "Secretary of Defense", "Dept of Defense", and "DoD" all resolve to
 * "Department of Defense". This keeps referenced_agencies arrays clean
 * and containment queries predictable.
 */
...
// Pre-computed lowercase → canonical map (used at runtime)
export const AGENCY_LOOKUP: Map<string, string> = new Map(
  RAW.map(([variant, canonical]) => [variant.toLowerCase(), canonical]),
)

export function extractAgencies(text: string): string[] {
  const lower = text.toLowerCase()
  const found = new Set<string>()

  for (const [variant, canonical] of AGENCY_LOOKUP) {
