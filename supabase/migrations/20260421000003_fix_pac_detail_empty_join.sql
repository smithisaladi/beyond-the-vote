-- Fix pac_detail: cmte_info CTE must always return a row (use aggregate)
-- so CROSS JOIN doesn't eliminate results when cmte_id isn't in fec_cmte_names.
-- Also show unmatched candidates (not in legislators table) in recipients list
-- using FEC candidate name data from the raw tables when available.
